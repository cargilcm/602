/*
 * jadvisor/school/COFC.java - SchoolAdapter for North Carolina State Univ.
...
public CofC () {
	_departments = new String[][] {
		{"CSCI","MATH","PHYS","GEOL","CHEM",}/*AGI", "ACC", "ADN", "AEE", "AFS", "ALS", "ANS", "ANT", "ARC",
...
private void readData(File f, String[] courses, List[] classes, int k) throws IOException  {
	...
	for (int i = 0; i < c2.length; i++) {
		...
		//System.out.println(_courses[k][i]+ " added to _courses");
	}
	...
	for (int i = 0; i < classList.size(); i++) {
		...
		_classes[k][n].add(c);
		//System.out.println(_classes[k][n]+ " added to _classes");
